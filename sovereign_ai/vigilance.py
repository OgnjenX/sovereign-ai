...
class VigilanceController:
    ...
    def __init__(
        ...
        self.debug = debug
        self.weights = {name: np.zeros(feature_dim, dtype=float) for name in self.field_names}
    ...
    def modulation(
        ...
        return float(np.clip(features @ self.weights.setdefault(field_name, np.zeros(self.feature_dim)), -0.2, 0.2))
    ...
    def learn(
        ...
        trace = VigilanceTrace(field_name, self.modulation(field_name, field_activation, mismatch, reward), mismatch, reward, float(np.linalg.norm(update)))
    ...
    def _features(self, activation: np.ndarray, mismatch: float, reward: float) -> np.ndarray:
        activation = np.asarray(activation, dtype=float)
        confidence = float(np.max(activation)) if len(activation) else 0.0
        dispersion = float(1.0 - np.linalg.norm(activation, ord=2)) if len(activation) else 1.0
        return np.array(
            [
                1.0,
